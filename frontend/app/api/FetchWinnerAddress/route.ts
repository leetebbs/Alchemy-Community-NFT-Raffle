...
export async function GET(request: NextRequest) {
    try {
        const raffleContractAddress = process.env.NEXT_PUBLIC_RAFFLE_CONTRACT_ADDRESS as `0x${string}`;

        if (!raffleContractAddress) {
            return NextResponse.json({ error: 'Raffle contract address is not defined in environment variables.' }, { status: 500 });
        }
    ...
    const [numberOfEntries, commitmentHash, month, isActive, createdAt, selectedWinnerIndex, winnerAddress] = data as [bigint, bigint, string, boolean, bigint, bigint, `0x${string}`];
        ...
        return NextResponse.json({ error: 'Failed to fetch winner address' }, { status: 500 });
    }
}